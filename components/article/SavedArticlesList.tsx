  ...
  return (
    <Tabs defaultValue="myArticles">
      ...
      <TabsContent value="myArticles">
        <div>
          {!articles ? (
            <div>You have not written any articles yet.</div>
          ) : (
            <div>
          ...
          {favorites.map((favArticle: any) => (
            ...
            >
                      ...
                      {(favArticle?.editorData as any)?.blocks.map(
                        (item: any, index: number) => {
                          if (item.type === "header") {
                            ...
                            return (
                              <div
                                key={`${favArticle.id}-header-${index}`}
                                className="font-bold text-xl"
                              >
                                {headerData.text}
                              </div>
              ...
              <CardFooter className="flex items-center justify-between gap-3 p-0 py-5">
                <div className="flex items-center gap-3 px-5">
                  <span className="text-xs text-muted-foreground">
                    {favArticle.readTime} min read
                  </span>
                  <span className="text-xs text-muted-foreground">
                    · {moment(favArticle.updatedAt).format("MMM D YYYY")}
                  </span>
                </div>
                <div className="flex items-center gap-3">
                  <DropdownMenu>
              ...
              <DialogContent className="shadow-md rounded-none flex flex-col gap-10 justify-center items-center py-48">
                <DialogHeader className="flex flex-col gap-5">
                  ...
                  <DialogDescription>
                    ...
                    recommendations that you receive in your feed or email
                    digest
                  </DialogDescription>
                </DialogHeader>
                <div className="flex justify-end gap-5">
            ...
            >
              <CardContent className="p-5">
                <div className="flex items-center gap-2 mb-3">
                  <div>
                    <Avatar>
                      <AvatarImage src={lastArticle?.article?.user?.image} />
                      <AvatarFallback>
                        <User2 />
                      </AvatarFallback>
                    </Avatar>
                  ...
                  <div className="font-medium">
                    {lastArticle?.article?.user?.name}
                  </div>
                </div>
                <div className="flex items-center justify-between gap-10">
                  <div>
                    <Link
                      href={`/article-details/${lastArticle.article.id}`}
                      className="hover:cursor-pointer flex flex-col gap-3"
                    >
                      {(lastArticle?.article?.editorData as any)?.blocks.map(
                        (item: any, index: number) => {
                          if (item.type === "header") {
                            const headerData = JSON.parse(item.data);
                            return (
                              <div
                                key={`${lastArticle.id}-header-${index}`}
                                className="font-bold text-xl"
                              >
                                {headerData.text}
                              </div>
                      ...
                      <div className="font-normal hidden md:block">
                        {lastArticle?.article.previewSubtitle}
                      </div>
                    </Link>
                  </div>
                  <div className="border">
                    <div className="relative size-16 sm:size-20 md:size-28 aspect-video flex justify-center items-center">
                      {lastArticle.article.image ? (
                        <Image
                          src={lastArticle?.article.image}
                          alt={"article-pic"}
                          fill
                          sizes="1"
                          className="w-full mx-auto"
              ...
              <CardFooter className="flex items-center justify-between gap-3 p-0 py-5">
                <div className="flex items-center gap-3 px-5">
                  <span className="text-xs text-muted-foreground">
                    {lastArticle.article.readTime} min read
                  </span>
                  <span className="text-xs text-muted-foreground">
                    ·{" "}
